...
export async function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/60 bg-white/70 backdrop-blur-xl dark:border-slate-800/60 dark:bg-[#09090b]/70 supports-[backdrop-filter]:bg-white/60">
      ...
      <div className="mx-auto flex w-full max-w-5xl flex-row items-center justify-between gap-3 px-4 py-2 sm:py-4">
        <div className="flex items-center gap-4">
          <Link href={`/${currentLocale}`} className="flex flex-col group">
            ...
            <span className="hidden sm:block text-xs font-medium text-slate-500 dark:text-slate-400">{t("common.tagline")}</span>
          </Link>
          <div className="flex">
            <ThemeToggle />
          </div>
